...
$(() => {
    ...
    $("#howdyImporter").html(
        $("#howdyImporter")
            .html()
            .replace(/{modifer}/g, apple ? "⌘" : "control")
    )
    refreshTimer()
    ...
    if (output[2].length) {
        ...
        if (
            navigator.geolocation.getCurrentPosition(location => {
                ...
                $("iframe")
                    .show()
                    .prop("src", prevAddress.length ? "https://www.google.com/maps/embed/v1/directions?origin=" + prevAddress + "&destination=" + address + "&mode=walking&key=AIzaSyBFw0Qbyq9zTFTd-tUY6dZWTgaQzuU17R8" : "https://www.google.com/maps/embed/v1/place?q=" + address + "&key=AIzaSyBFw0Qbyq9zTFTd-tUY6dZWTgaQzuU17R8")
            })
        ) {
        } else {
    ...
    if (localStorage.schedule) {
        ...
        for (var i = 0; i < 7; i++) {
            ...
            for (var j = 0; j < schedule[i].length; j++) {
                ...
                var potentialHeight = (schedule[i][j].end - schedule[i][j].start) * 0.015
                daysHtml += "<a href=#activityEditor id=activityButton" + i + "-" + j + ' class="activity btn waves-effect waves-light ' + color + '"data-position=bottom onclick=editActivity(' + i + "," + j + ") style=height:" + (potentialHeight < 36 ? 36 : potentialHeight) + "px;top:" + ((schedule[i][j].start - earliestActivityTime * 3600) * 0.015 + 27) + "px>" + schedule[i][j].name + "</a>"
            }
            var timeSeconds = new Date().getSeconds() + new Date().getMinutes() * 60 + new Date().getHours() * 3600
            if (i == new Date().getDay() && earliestActivityTime * 3600 <= timeSeconds && timeSeconds <= latestActivityTime * 3600) daysHtml += '<span class="separator currentTime" style=top:' + ((timeSeconds - earliestActivityTime * 3600) * 0.015 + 26) + "px></span>"
            daysHtml += "</td>"
        }
...
function editActivity(day, activityNumber) {
    ...
    $("#saveActivity")
        .unbind("click")
        .click(() => saveActivity(day, activityNumber))
    $("#deleteActivity")
        .unbind("click")
        .click(() => deleteActivity(day, activityNumber))
}
...
function saveActivity(day, activityNumber) {
    ...
    $("#activityDay")
        .val()
        .forEach(day => schedule[day].push(new activity($("#activityName").val() ? $("#activityName").val() : "untitled", convertToSeconds($("#activityStart").val()), convertToSeconds($("#activityEnd").val()), $("#activityLocation").val())))
    $("#activityName").val("")
    $("#activityLocation").val("")
    $("#activityStart").val("")
    $("#activityEnd").val("")
